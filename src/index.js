...
export function withAnimated(Component, animateClass) {
	const withAnimated = (props) => {
		...
		return (<Component {...rest} style={styleElem} className={`${className} ${ (classes.length !== 0) ? "animate__animated" : ""} ${classes.join(" ")}`}/>)
	};
...
export function withAnimatedGroup(Component, animateOptions) {
	...
	withAnimatedChildren.displayName = `withAnimatedChildren(${getDisplayName(Component)})`;
